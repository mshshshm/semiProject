  ...
  #replyList .comment_output {
	...
	function boardDel() {
		if(confirm("글을 정말로 삭제하시겠습니까?")) {
			location.href = "/campus/qna_board/delete?no=${vo.boardno}";
		}
	}
	...
	$(function() {
		...
		function replyList() {
			$.ajax({
				url:'${pageContext.servletContext.contextPath}/qna_boardReply/list',
				data:{no:${vo.boardno}},
				type: "GET",
...
<body>
    <main>
        <div class="board_wrap">
            ...
            <div class="bt_wrap">
                <a href="/campus/qna_board/list?nowPage=${pVO.nowPage}<c:if test="${pVO.searchWord!=null}">&searchKey=${pVO.searchKey}&searchWord=${pVO.searchWord}</c:if>">목록</a>
                <c:if test="${vo.userid==logId}">
					<a href="/campus/qna_board/edit?no=${vo.boardno}">수정</a>
