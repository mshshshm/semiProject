            ...
            <div class="board_list_wrap">
                ...
                <div class="board_list">
                    ...
                    <div>
                        ...
                        <div class="title"><a href="/campus/board/view?no=${bVO.boardno}&nowPage=${pVO.nowPage}<c:if test='${pVO.searchWord!=null}'>&searchKey=${pVO.searchKey}&searchWord=${pVO.searchWord}</c:if>">${bVO.subject}</a></div>
					...
					<ul>
						...
						<c:forEach var="p" begin="${pVO.startPage }" end="${pVO.startPage+pVO.onePageCount-1}">
							<c:if test="${p<=pVO.totalPage }">
								...
								<a href="/campus/board/list?nowPage=${p}<c:if test='${pVO.searchWord!=null}'>&searchKey=${pVO.searchKey}&searchWord=${pVO.searchWord}</c:if>">${p}</a>
								</li>
						    </c:if>
						</c:forEach>
						<!-- next page -->
						<c:if test="${pVO.nowPage<pVO.totalPage }">
							<li><a href="/campus/board/list?nowPage=${pVO.nowPage+1}">next</a></li>
						</c:if>
						<c:if test="${pVO.nowPage>=pVO.totalPage }">
							<li>next</li>
						</c:if>
                ...
                <div class="bt_wrap">
                    <a href="/campus/board/write" class="on">등록</a>
                    <!--<a href="#">수정</a>-->
                </div>
            </div>
        </div>
