  ...
  #replyList .comment_output {
	...
	function boardDel() {
		if(confirm("글을 정말로 삭제하시겠습니까?")) {
			location.href = "/campus/media_board/delete?no=${vo.boardno}";
		}
	}
	...
	$(function() {
		...
		function replyList() {
			$.ajax({
				url:'${pageContext.servletContext.contextPath}/media_boardReply/list',
				data:{no:${vo.boardno}},
				type: "GET",
		...
		$("#replyForm").submit(function() {
			...
			var params = $(this).serialize();
				...
				$.ajax({
					url: "/campus/media_boardReply/write",
					data: params,
					type: "POST",
					success:function(result) {
						console.log(result);
		...
		$(document).on('submit', '#replyList form', function() {
			event.preventDefault();
			var params = $(this).serialize();
			var url = "${pageContext.servletContext.contextPath}/media_boardReply/editOk";
		...
		$(document).on('click', '#replyList input[value=Del]', function() {
			if(confirm("삭제하시겠습니까?")){
				...
				$.ajax({
					url: "${pageContext.servletContext.contextPath}/media_boardReply/delete",
					data: {
						replyno: replyno
					},
					type: "GET",
...
<body>
    <main>
        <div class="board_wrap">
            ...
            <div class="bt_wrap">
                <a href="/campus/media_board/list?nowPage=${pVO.nowPage}<c:if test="${pVO.searchWord!=null}">&searchKey=${pVO.searchKey}&searchWord=${pVO.searchWord}</c:if>">목록</a>
                <c:if test="${vo.userid==logId}">
					<a href="/campus/media_board/edit?no=${vo.boardno}">수정</a>
